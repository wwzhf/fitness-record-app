...
| v1.0.5 变更（2026-08-29） | 日历中有健身记录的日期整格加淡色圆角底色（主题三号色 12% 透明度），与无健身的日子直观区分 |
| v1.0.6 变更（2026-08-29） | 修复 Excel 导入 bug：转换工具误用 toordinal() 作 dateEpochDay（日期落到公元 3994 年）。DB v1→v2 一次性迁移删除 dateEpochDay > 100000 的异常体重行；转换工具已改用正确的 Unix epochDay |
| v1.0.7 变更（2026-08-29） | 导入语义从"合并追加"改为**覆盖替换**：导入时在单事务内先清空现有全部数据（组→训练→动作→体重），再写入备份内容；动作保留备份中的归档状态。同文件重复导入结果幂等 |
| v1.0.8 变更（2026-08-29） | 导入语义再改为**按日期覆盖合并**：文件中出现的日期，当天训练（含组）与体重以文件为准覆盖；文件未涉及的日期、动作，App 现有数据保留。动作按名字覆盖/新增（保留归档状态）。重复导入幂等，手动数据不再被误删 |
| v1.0.9 变更（2026-08-29） | 修复趋势图 x 轴刻度重叠：刻度数量随数据跨度自适应（跨度 1 天 2 个、2 天 3 个、≥3 天 4 个）并去重 |
| v1.1.0 变更（2026-08-29） | 编辑已结束训练时可修改时长：训练页时长旁 ✏️ 弹窗输入分钟数（预填向上取整，校验正整数），按"开始时间 + N 分钟"更新结束时间；进行中训练不受影响 |
| v1.2.0 变更（2026-08-29） | 工程：Release 开启 R8 压缩与资源收缩（APK 17.4MB→1.4MB，debug 签名直接分发）、Room schema 导出入库（2/3.json）+ MigrationTestHelper 迁移路径测试、组数据查询改 Flow 响应式（删除手动刷新）、CalendarScreen 拆分为 4 个文件、新增 5 个 VM 测试。功能：训练容量统计（日历详情行 + 趋势页体重/训练量切换曲线）、动作历史与 PR 页（最大重量/最大单组容量/PR 标记）、添加动作按最近使用排序、会话备注字段（schema v3，备份兼容） |
| v1.2.1 变更（2026-08-29） | 纯自重动作（全部组 0 重量）的历史页不再显示 重量PR/容量PR，改按「组数PR」标记组数最多的训练，头部显示"最多 X 组"；混合动作维持原 PR 规则 |
| v1.2.2 变更（2026-08-29） | 纯自重动作的 PR 修正为「次数PR」：单组次数最多的组打「次数PR」标记（并列都标），头部显示"最多 X 次"；组数PR 移除 |
| v1.2.3 变更（2026-08-30） | 自重支持打通：组录入/编辑校验放宽为重量 ≥ 0（0 = 自重），展示层新增 formatSetSummary/formatSetRow（0kg 显示"自重×N"），纯自重「次数PR」分支由死代码变为可用；动作历史页改 Room Flow 响应式（新增 observeExerciseHistory = 会话流+组流 combine，动作名 observeById），训练页补录组返回即刷新；重复添加动作发 Snackbar「该动作已在本次训练中」 |
| v1.2.4 变更（2026-08-30） | 会话内动作排序：训练页动作卡片新增拖动柄（sh.calvin.reorderable 3.1.0，长按拖动、边缘自动滚动、震动反馈），拖拽中用本地列表驱动、松手后按展示顺序单事务把 exerciseOrder 重写为 1..N（无移动跳过，失败回退并提示）；挂起动作（未录组）固定末尾不参与；已结束训练同样可重排；日历详情按 exerciseOrder 分组自动跟随 |
| v1.2.5 变更（2026-08-30） | 修复历史页头部"最大重量"并列取组 bug：bestWeightSet 原用 firstOrNull（列表序在前者优先），同重量的并列组改为取次数最多的一组（bestWeightSetOf 纯函数 + 3 测试）；PR 标记维持"并列都标"不变 |
| v1.2.6 变更（2026-08-30） | 重量PR 标记改为只标 bestWeightSetOf 选出的代表组（与头部「最大重量」引用完全一致），同重量并列中较差的组不再标 PR；容量PR / 次数PR 维持并列都标（并列值本身无差异） |
...
**导入算法**（v1.0.8 起为**按日期覆盖合并**语义；单事务，任一步失败整体回滚并提示错误，不产生半份数据）：

1. 校验 `schemaVersion`，不匹配（更大版本）则拒绝。
2. 动作：按 `name` 匹配——本地已有则覆盖元数据（`createdAt`、`isArchived`），否则插入；本地未被备份涉及的动作保留。
3. 体重：逐条按 `dateEpochDay` upsert（文件涉及的日期覆盖，其余保留）。
4. 训练与组：按会话 `startTime` 所在日分组——文件涉及的每个日期，先删除本地当天全部会话（级联删组，窗口为 `[当日零点, 次日零点)`，DST 安全），再插入文件中该日的会话与组；文件未涉及的日期保留本地会话。
5. 组的 `sessionIndex` 在写入前统一校验越界。

即：文件涉及的日期以文件为准，其余数据原样保留；同文件重复导入结果幂等。导出经 SAF `CreateDocument("application/json")`，导入经 `OpenDocument`，均无需存储权限。
