...
| 日历 | CalendarScreen | 自绘月历网格（7 列 `Canvas`/`Layout`，不引第三方日历库）；月份左右切换；每格：日期数字、体重 kg 小字（有则显示）、健身标记（右下角小圆点或标题首字）；点击日期弹出 BottomSheet 当日详情：当日体重 + 各次健身卡片（可展开 动作→各组 重量×次数、开始/结束时间与时长）；已结束的会话**只读**，仅提供「删除该次记录」（二次确认，级联删组），不提供编辑历史组数据（后续需要再加） |
...
3. 训练进行页：
   - 顶部：标题 + 已用时计时器（`now - startTime` 每秒刷新；不依赖前台服务，App 在后台或被杀后回来都按 `startTime` 重算，天然正确）。
   ...
   - 录入一组：校验 `weightKg > 0` 且 `reps > 0`（否则「添加」禁用）；保存后**重量保留、次数清空**，方便连续录组。
