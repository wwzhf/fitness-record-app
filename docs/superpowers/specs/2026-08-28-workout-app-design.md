...
**导入算法**（v1.0.7 起为**覆盖替换**语义；单事务，任一步失败整体回滚并提示错误，不产生半份数据）：

1. 校验 `schemaVersion`，不匹配（更大版本）则拒绝。
2. 清空现有全部数据（顺序：组 → 训练 → 动作 → 体重，子表在前避免外键冲突）。
3. 插入动作（按备份中的归档状态）。
4. 插入体重。
5. 插入会话（生成新 id）。
6. 组：按 index 映射生成 `sessionId` / `exerciseId` 后插入。

即：导入完成后 App 数据与备份文件内容完全一致（同文件重复导入结果幂等）。导出经 SAF `CreateDocument("application/json")`，导入经 `OpenDocument`，均无需存储权限。
